...
<script>
import mock_1_json from "@/mock/atm/atm_mock_1.json";
import mock_2_json from "@/mock/atm/atm_mock_1.json";
...
export default {
  ...
  data() {
    return {
      options: [
        { value: "1", label: "state transition testing" }
      ],
      value: "1",
      tableData: [],
      loading: false,
  ...
  methods: {
    ...
    doTest() {
      ...
      if (this.value === "1") {
        newData = {
          atm_test_list: mock_1_json,
        };
      } else {
        newData = {
          atm_test_list: mock_2_json,
        };
      }
      const _this = this;
      this.loading = true;
